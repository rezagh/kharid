...
//		Map<String,Double> m = exchangeBiz.getExchangeRates();
...
//	public String calculateSimplePrice(String foreignCurrency, String irrRate){
//		Double dFrn; Integer iRate;
//		try{
//			dFrn = new Double(foreignCurrency);
//			iRate = new Integer(irrRate);
//		}catch(Exception e){
//			return "-";
//		}
//		
//		Long out = priceBiz.calculateSimplePrice(dFrn, iRate);
//		
//		return out.toString();
//	}
	...
	public Map<String,String> calculateFinalPrice(PriceVO priceVO){
		...
		if(StringUtils.isBlank(priceVO.getWeight()))
			finalPrice.put("nopost",calculateFinalPrice(priceVO,null));
...
//	}
	...
	public CreditCardVO getMasterCardDetail(int amount, int audRate){
		CreditCardVO vo = priceBiz.getMasterCardDetail(CreditCardTypeEnum.Master_Woolworths,amount, audRate);
		vo.setPrice(PriceExchangeUtil.commaSeparate(vo.getPrice()));
		vo.setPhysicalPrice(PriceExchangeUtil.commaSeparate(vo.getPhysicalPrice()));
		return vo;
	}
